...
export default class DelugeKit extends PartyElement {
	...
	constructor() {
		...
		midiinput?.addEventListener("midimessage", (event) => {
			...
			if (type == MIDI.MidiMessage.NoteOn) {
				let note = data[1];
				let velo = data[2];
				let sound = this.kit.sounds[note - 53];
	...
	connectedCallback() {
		...
		this.sourcesElement.addEventListener("audition", (event) => {
			if (event.target.sound.playmode == "group") {
				for (let sound of this.kit.sounds) {
					if (sound.playmode == "group" && sound != event.target.sound) {
						sound.stop();
					}
				}
			}
	...
	update() {
		...
		for (let [index, sound] of kit.sounds.entries()) {
			let el = /** @type {import("./sound-element.js").default} */ (
				this.sourcesElement.children[index]
			);
